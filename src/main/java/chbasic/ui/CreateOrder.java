...
public class CreateOrder {
	...
	public CreateOrder(String create_order_or_path, UIDriver driver)
	...
	public boolean selectCreateOptions(String order_type, String req_type,
			String supplier) throws XPathExpressionException, IOException,
		...
		try {
			...
			new Select(iedriver.findElement(cn.getLocator(order_type_locator)))
					.selectByVisibleText(order_type);
			Thread.sleep(2000);
			...
			new Select(iedriver.findElement(cn.getLocator(req_type_locator)))
					.selectByVisibleText(req_type);
			Thread.sleep(2000);
			...
			new Select(iedriver.findElement(cn.getLocator(supplier_locator)))
					.selectByVisibleText(supplier);
            
			xpath = "/create_order/CREATE_ORDER_BUTTON";
			String Create_order_button_locator = uiPopulation
					.getNodeValByXpath(xpath);
			logger.info("Create_order_button Locator::"
					+ Create_order_button_locator);
			Thread.sleep(2000);
			iedriver.findElement(cn.getLocator(Create_order_button_locator))
					.click();
			return true;
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public boolean selectCreateOptionsWithTemplate(String order_type,
			String req_type, String supplier, String template_name)
		...
		try {
			...
			String order_type_locator = uiPopulation
					.getNodeValByXpath(xpath_temp);
			logger.info("Order_type Locator::" + order_type_locator);
			new Select(iedriver.findElement(cn.getLocator(order_type_locator)))
					.selectByVisibleText(order_type);

			xpath_temp = "/create_order/select_order/REQUEST_TYPE";
			String req_type_locator = uiPopulation
					.getNodeValByXpath(xpath_temp);
			logger.info("Request_type Locator::" + req_type_locator);
			new Select(iedriver.findElement(cn.getLocator(req_type_locator)))
					.selectByVisibleText(req_type);
			...
			String supplier_locator = uiPopulation
					.getNodeValByXpath(xpath_temp);
			...
			String template_locator = uiPopulation
					.getNodeValByXpath(xpath_temp);
			logger.info("Order_type Locator::" + template_locator);
			new Select(iedriver.findElement(cn.getLocator(template_locator)))
					.selectByVisibleText(template_name);
